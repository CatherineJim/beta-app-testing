...
const Test = () => {
  ...
  useEffect(() => {
    (async () => {
      const { data, error } = await getAppById(id);
  ...
  const handleAppView = () => {
    const windowFeatures = "left=100,top=100,width=auto,height=auto";

    window.open(
      "https://appetize.io/embed/gagh5zkimn34ws3vwmde7mlbku?device=pixel4&osVersion=11.0&scale=75",
      windowFeatures,
      "popup"
    );
  };
  ...
  return (
    <div className="grid lg:grid-cols-8 w-full">
      ...
      {app && (
        <div
          className={`${device === "android" ? "col-span-5" : "col-span-4"}`}
        >
